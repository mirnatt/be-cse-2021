...
{{{unit}}}
| UNIT IV | POINTERS   | 6 |
Pointers: pointer operators -- pointer operations – pointer to an
array -- array of pointers -- Dynamic memory allocation -- pointer to
a function -- passing pointers to functions --problems in using
pointers.
